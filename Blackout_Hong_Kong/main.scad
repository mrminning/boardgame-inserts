...
// Coins
module coins() {
  tokenBox(size=[(inner_well_length_bottom - 101) / 2, inner_well_width, top_layer_height], hexBottom=no_hex, corner=3, containersY=2, bottomCorner=3);
}
...
module playerbox() {
  tokenBox(size=[inner_well_length_top / 3 - 0.3, inner_well_width / 2 - 0.5, top_layer_height], hexBottom=no_hex, corner=3, bottomCorner=3);
}

// Scout tiles, Dice + start player
module scoutTiles() {
  tokenBox(size=[inner_well_length_top / 3 - 0.3, inner_well_width, top_layer_height], hexBottom=no_hex, containersY=[120], corner=3, bottomCorner=3);
}
